...
import { getCandidates, getElection } from '../services/electionService';
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white shadow-lg rounded-lg p-6 mb-8">
        ...
        <div className="mb-4">
          ...
          <span className={`ml-2 text-sm font-bold px-3 py-1 rounded ${isElectionCompleted ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
        ...
        {!isElectionCompleted && (
          <p className="text-sm text-yellow-600 mb-4">
            Note: The election is still in progress. Results may change as more votes are cast.
          </p>
        )}
      </div>
      ...
      <div className="bg-white shadow-lg rounded-lg p-6">
        ...
        {candidates.length === 0 ? (
          <p className="text-gray-600">No candidates have been nominated for this election.</p>
        ) : (
          <>
            ...
            {isElectionCompleted && candidates.length > 0 && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                ...
                <p className="text-green-700 font-medium text-xl">{candidates[0].name}</p>
                <p className="text-green-600">
                  {candidates[0].voteCount} votes ({totalVotes > 0 ? Math.round((candidates[0].voteCount / totalVotes) * 100) : 0}%)
                </p>
              </div>
            )}
              ...
              {candidates.map((candidate, index) => {
                ...
                return (
                  <div key={index} className="border rounded-lg p-4">
                    ...
                    <p className="text-gray-700 text-sm">{candidate.platform}</p>
                  </div>
                );
              })}
            </div>
